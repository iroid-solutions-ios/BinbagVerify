...
//        bgImageView?.layer.cornerRadius = (bgImageView?.frame.size.height ?? 0)/2
    ...
    func frameForImageView() -> CGRect {
        return CGRect(x: (loadingView?.frame.origin.x ?? 0)+15, y: (loadingView?.frame.origin.y ?? 0)+8, width: (loadingView?.frame.size.width ?? 0)-30, height: (loadingView?.frame.size.height ?? 0)-30)
    }
